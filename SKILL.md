...
- squeezed-out function words（虚词被压掉）: 这一条查不了字符串，要逐句念出声——`位置得我自己猜` 念出来会多一个字（`还` / `只能`），`猜错半秒整段重新混一遍` 少了 `就得`。常被压掉的是 `还` / `只能` / `才` / `就` / `就得` / `都` / `也` / `又` / `非得` / `根本`
