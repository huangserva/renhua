...
黄佬 2026-09-21 定的总纲，三道的顺序不许调：先判断语境，再判断句子本身，最后才轮到词汇。他的原话：「一定要按照这样的结构去去AI味，它才是能成功的。这是一个最基本的去做好语文的一个学习的事情，就是中文语文的表达」。

顺序反过来做就会翻车：盯着词表逐句改，同一个字会被改错方向，整句不通的地方反而漏掉。

### 贯穿三道：简洁是目标，砍词不是手段

三道都在往同一个方向使劲——把观点简洁扼要地说出来。短句是对的，拖沓才是问题。要分清的是怎么把句子变短。

黄佬 2026-09-21 原话：「要表达清楚，是尽量压缩词汇能够做到的，而不是通过把动词切割一半、把什么切割一半去完成的……我们是要压缩词汇，但是是简洁扼要地表达出观点；而我们这种去掉一半的方法，它表达不出观点，这才是最核心的」。

该删的，删完观点还在：

- 空形容词和废话修饰：`强大的`、`便捷的`、`显著的`
- 不传信息的过渡和铺垫
- 前面已经说过一遍的话
- 只剩语感、不传信息的劲儿词
...
- 动词本身。`估` 砍自 `估计`，砍完读者不知道在估什么
- 主谓宾任何一个成分
- 必要的虚词：`还`、`只能`、`就得`、`去`

判据是砍完之后问一句：这句话还表达得出那个观点吗？

> `今天可以删了` —— 删的是什么上文交代过，观点完整，保留。

> `我只能自己听着估` —— 估什么？没写，观点没表达出来，要改。

同一个单字动词，一句留一句改，分界就在这里。压缩词汇是对的，压缩到观点漏出去就错了。

所以「把长句拆开」「段落短一点」这类要求跟这个 skill 不冲突，可以照做。冲突的只有一种：为了凑字数上限去砍词。

### 第一道：语境

先把整篇读完，再动第一个字。要弄清楚三件事：

1. 这篇在讲什么事，给谁看，什么场合（推特短贴、公众号长文、商单交付稿、内部汇报）。
2. 每句话在上下文里承担什么——是在报结果、讲步骤、吐槽，还是在给判断。
3. 这句话原本是说出口的那种，还是写下来的那种。口语场合的短句和书面场合的完整句，标准不一样。

语境决定同一个字展开成什么词。下面这两句里都是 `对`：

> 跟它对着测试了一轮 → 跟它**对比**着测试了一轮

> 一个事件一个事件地对时间点 → 一个事件一个事件地**核对**时间点

同一个单字，一句展开成 `对比`，一句展开成 `核对`。词表给不出这个答案，只有读懂上下文才知道这里在干什么。

语境也决定一个词要不要动。`今天可以删了` 里的 `删` 按词汇那道该补成 `删除`，但这句是说出口的短句，写成 `今天可以删除了` 就成了对话框提示音。这种照旧保留。

还有一条：词放进这个句子不顺，就换一个说法，不要靠缩写硬塞，也不要硬把表里的替换词按上去。黄佬原话：「估计就是估计，或者估计这个词在这个句子不行，就换一个方式表达」。


#### 整篇结构体检

第一道还要把整篇端起来看一次。逐句改稿永远看不到下面这三样：

- 开头承诺了几条，后面就要有几节兑现。开头说「三个问题」、正文只讲了两个，缺的那个要么补上，要么把开头改掉。
- 主论收完之后最多一节收尾。收完又接着讲同一个意思的段落，删掉。
- 同一个意思有没有在两个地方各说了一遍。最常见的是术语解释框跟正文重复。只查连续 20 字以上的逐字重复，作者故意的呼应、换个说法的复述、开头埋钩结尾回扣都不算。

#### 语境读数（一句话写下来，后两道要用）

第一道读完要留一个读数，写下来，别只放在脑子里：

**这段稿子是说出口的那种，还是写下来的那种。**

只留这一个二分，不做评分，不列维度表。它在两个地方生效：第二道第 2 问判动词有没有被砍成单字的时候，以及第三道 truncated verbs 那一条。口语场合的短句按口语的标准看（`今天可以删了` 保留），书面场合按完整句的标准看。读数拿不准就按书面场合处理。
...
1. 谁做的？主语在不在。
2. 做了什么？动词在不在，有没有被砍成单字（按第一道的语境读数判：口语场合放宽，书面场合从严）。
3. 对什么做的？该带的宾语在不在。
4. 念出声，会不会多一个字？被压掉的 `还` / `只能` / `就得` 补回去。
5. 这句里最抢眼的那个词，换成最平的说法，句子少了什么信息没有？没少，那个词就是在表演，换掉它。

第五问是所有劲儿词禁令的母规则。`锁死`、`碾压`、`炸裂`、`全在里头` 读着像 AI，是因为 AI 爱把它们摆进句子里做展示，而那个位置本来只需要一个平词。人当然也会说 `锁死`。要看的是它在这句话里在干什么：传信息，还是使劲。

举例：`把人物锁死在第一帧`，平说法是 `让人物保持第一帧的样子`。换完信息一点没少，那 `锁死` 就是在表演。

第五问也是词表长不出来的那部分。新词天天有，AI 摆词的位置不变，抓位置比抓词管用。

### 第三道：词汇

Hard Bans 各节和 Before Returning 清单。三道都要过，顺序不能反：先读懂语境，再把句子改成人话，最后用词表扫一遍捡漏。

词表命中了也要退回第一道问一句：在这个语境里，这个词是在传信息还是在使劲。人也会说 `锁死`，问题从来在 AI 把它摆的那个位置上。

只有几个词已经按第二道第五问推导定案，列成无条件禁用，不用每次重新判断：`写死`（黄佬原话「这个永远不要写」）和「动词 + 死」那一族。这是为了省事，词汇的地位还是第三道。

## 常见误判

这一节只收方法层面的误判——该按什么顺序做、哪一道不能跳。单词级的例外照旧留在各自小节里，不搬到这里，否则两处会各说各话。

**❌ 去 AI 味就是拿禁用词表扫一遍。**
✅ 词表是第三道。压缩型、电报型、没主语的句子，词表里一个都查不到。黄佬原话：「我们不能够去审这些词汇，你审的这词汇就是AI味」。

**❌ 句子写短就是口语，写长就是书面腔。**
✅ 短和完整是两件事。15 个字的句子照样可以主谓宾齐全。短要靠删掉不传信息的词来拿，不能靠砍掉传信息的词的一半。

**❌ 补全就是把句子写长。**
✅ 简洁扼要地表达出观点才是目标。补的是被砍掉的那半个词和被压掉的虚词，不是往句子里加内容。

**❌ 框架定下来、语境统一了，词汇自然就好。**
✅ 实测反驳过。乔木那套 skill 的框架在成品里是真落地的（开头埋三条预告、后文三节一一兑现），句子层也站得住（双音节动词一例没砍），但两篇成品合计仍有「不是A而是B」十处、「本质上」两处，打架比喻还写进了小标题。前两道做好，第三道那张表省不掉。

**❌ 三遍自审可以合成一遍，边写边看顺手就改了。**
✅ 必须分开做。写的人刚把句子造出来，回头再读看到的是自己想表达的意思，看不见句子里还留着壳子。

**❌ 所有口语词、所有不规整的说法都是 AI 味。**
✅ 作者手改稿里的口语词不算。`其实` 就是黄佬自己会写的词，不删。要清的是 AI 造出来的顺滑感，不是作者本人的粗糙感。

**❌ 禁用词命中了就直接换掉。**
✅ 命中之后要退回第一道问一句：在这个语境里，这个词是在传信息还是在使劲。人也会说 `锁死`，问题在 AI 把它摆的那个位置上。只有已经定案的无条件禁用项（`写死` 和「动词 + 死」那一族）不用每次重新判断。
...
- 只剩名词加趋向词或者方位词，动词整个不见：`一段脚本进去`、`三个参数进去，结果出来` → 把动作补回来，`我把一段脚本扔进去`、`我把三个参数传进去，它把结果吐出来`。退一步至少也要有动词：`将一段脚本扔进去`
...
黄佬 2026-09-21 当场打回的两句，永久当反例（他原话：「我们不能够去审这些词汇，你审的这词汇就是AI味，而且要有主谓宾」）：

- 「一段脚本进去，人声、音效、环境声一起出来」：没有动词，也没有人。谁把脚本放进去？改成 `我把一段脚本扔进去，它把人声、音效、环境声一次全生成出来`。
- 「位置得我自己猜」：电报腔，人不会这么说话。改成 `音效该放在第几秒，我只能自己听着估`。

同一类的电报腔还有 `CosyVoice3 出人声，demucs 拆音效，ffmpeg 混音`（改成 `我用 CosyVoice3 生成人声，用 demucs 把音效拆出来，用 ffmpeg 混音`）、`四个终端窗口伺候一段旁白`（改成 `做一段旁白我要开四个终端窗口`）。判据：把这句读出声，想象是在饭桌上跟朋友讲，讲不出口的就是电报腔。**扫词表抓不到这类句子，只能逐句读出声。**

查法：每个自然段的第一句带主语；段落中间动作主体从 `我` 换成脚本、服务、机器的时候重新点出主语；看到句子以动词收尾（报错、失败、通过了、挂了），回头检查动作的对象有没有写出来。

#### 第二层：双音节动词砍成单字

双音节动词砍成一个字，绝大多数情况要补全，范围不限于下面这张表。但这条**不一刀切**——判据是砍完之后这句话还表达得出那个观点（见上面「简洁是目标，砍词不是手段」），表达得出的照旧保留（`今天可以删了`）。黄佬 2026-09-21 原话：「估计就是估计，或者估计这个词在这个句子不行，就换一个方式表达！而不是把动词缩写」。

为什么这一类一眼就能认出来：那句话本来就该用两个字，你偏偏写了一个字，读的人看到的是「这里在省字」。黄佬原话：「明明是几个字或者两个字来表达的，你非要写一个字，那当然就觉得你为了省 token 还是省什么东西去做，这就是一眼就能看出来，这就是 AI 的行为」。人写字不会为了省这一个字把话说别扭，机器会。
...
- `传` → `上传` / `传输`
- `存` → `保存` / `存储`
- `估` → `估计` / `估摸`（`听着估` → `听着去估摸`。注意不是 `听着估计`，连动式缺 `去` 照样别扭，见下方 Squeezed-out function words）
- `对`（核对义）→ `核对` / `对照`（`对时间点` → `核对时间点`）
- `接`（接入义）→ `接入`（`再接一个模型` → `再接入一个模型`）
- `混`（混音义）→ `混音`（`重新混一遍` → `重新混音一遍`）

砍还是不砍，先过「简洁是目标，砍词不是手段」那一节的判据：砍完之后这句话还表达得出那个观点吗？表达得出就留，表达不出就补全。
...
- 本身就是单音节完整词的动词：`跑`、`跑通`、`拉到本地`、`锁`、`加锁`。它们不是任何双音节词的缩写，携带准确的工程含义，不走这节的任何检查。（`写死`、`锁死` 这一族不在此列，黄佬 2026-09-21 永久禁用，见下方 Banned words）

判据不是查表，是读出声：这个单字在口语里能不能独立当动词用。`用`、`看`、`写`、`拆`、`扔`、`念`、`唱`、`量`、`开`、`走`、`拉` 平时就这么说（`我看一眼`、`我扔进去`），保留；`估`、`对`（核对）、`接`（接入）、`混`（混音）、`测`、`查`、`改`、`删`、`装`、`配`、`传`、`存` 口语里说的是双音节（`我估计`、`核对一下`、`接入进来`），单字就是砍了一半，补全或者换说法。拿不准的按砍了一半处理，宁可多写一个字。
...
> 我把模型拉到本地跑通了一遍，端口固定成 8190，省得每次修改配置。

`拉到本地`、`跑通` 都保留。`跑通` 说的是整条流程走完没有报错。hardcode 这个意思写成 `固定成` / `定成` / `直接写成 8190`，不写 `写死`（黄佬永久禁用）。

### Banned words:「动词 + 死」这一族（黄佬永久禁用，2026-09-21）

黄佬原话：「写死 这个永远不要写！！！太AI味」。后来又点名 `锁死`：这两个是同一族，`锁死` 比 `写死` 更明显，因为它已经被用烂了。

整个「动词 + 死」的强调构词一律不用，任何语境都不用，包括 hardcode 这个技术含义：

`写死`、`锁死`、`焊死`、`钉死`、`封死`、`定死`、`咬死`。

这个 `死` 字不传信息，它在加语气。把 `死` 去掉之后，句子要么意思不变（说明它本来就多余），要么就得写清楚到底固定成什么样、固定到哪个值——后面这个才是读者要的东西。

改法：

- `端口写死在 8190` → `端口固定成 8190`
- `顺序写死` → `顺序一条条定好`
- `写死的配置` → `固定的配置` / `hardcode 的配置`（英文原词可以）
- `身份全片锁死` → `整片保持同一个人`
- `参数锁死` → `参数固定住` / `参数不再改动`
- `机位完全锁死` → `机位全程不动`
- `流程焊死` → `流程定下来不再改`
...
保留：`锁` / `加锁` 单独用、准确描述锁机制或者权限控制的时候照旧保留（`这张表加了锁`、`请求被限流锁住`）。禁的是后面挂一个 `死` 当强调。

### Squeezed-out function words（虚词被压掉，黄佬永久规则，2026-09-21 定）

主谓宾都在、句子也通，但口语里一定会出现的那个副词或者连接词被压掉了。压的动机是让句子显得紧凑有力，压完之后人不会这么说出口。这类 AI 味在禁用词表里查不到，只能逐句念出来找。

判据：把这句话念出声，问自己真要说它的时候会不会多一个字。会多，就把那个字补回去。

常被压掉的：`还`、`只能`、`才`、`就`、`就得`、`都`、`也`、`又`、`非得`、`根本`、`去`。

其中 `去` 单独说一类：连动式缺 `去`。两个动词连用、前一个带 `着` 的时候，第二个动词前面要有 `去`，省掉就别扭。

黄佬原话：「要加一个『去』字，『去估计』，而不是『我自己听着估计』，这也是非常奇怪的。这是最基本的语文都没有学好」。

Bad:

> 我只能自己听着估。

这句先砍了动词（`估` → `估摸`）。补成 `我只能自己听着估摸` 还是别扭，缺的是 `去`。

Better:

> 我只能自己听着去估摸。

同类：`盯着调` → `盯着去调整`；`对着改` → `对着去修改`。

界限：前一个动词只是在交代方式的时候不用加，`笑着说`、`躺着看`、`低着头走` 本来就顺。要加 `去` 的是「一边做着这个，一边摸索那个」的句子，第二个动词带着使劲的意思。判据还是念出声。
...
被压掉的 `就得` 承载的是后果，压掉之后因果关系要读者自己脑补。
...
不是每个 `得` 前面都要加 `还`。判断依据是念出来顺不顺，不是套公式。`这个得改` 本来就是完整的口语，不用动。
...
判据是看这个词后面接的是什么：后面接具体对象、数字或者数得出来的清单，保留；后面接大词，整条尾巴删掉。这跟 Performative hype verbs 那节保住 `锁`、`跑通` 用的是同一把尺子。
...
2. 第二遍，重读。把第一遍的结果当成别人交来的稿子，从头**念出声**读一遍，按「判断顺序」第二道的五问逐句过（动笔之前先把第一道的语境读一遍）。念出声是硬要求，默读抓不到压缩型的 AI 味。这一遍不许只对着 Before Returning 的词表扫——词表只抓得到已知的词，虚词被压掉、动词整个不见、句子压成电报体这几类，词表里一个都查不到。读起来像金句、显得精妙、显得顺口的地方先标记出来，再逐个检查里面的形容词、动作词、评价词有没有传具体信息；只剩语感、不传信息的删掉，作者自己的态度词和粗糙说法保留下来。按 Audit Mode 的格式列出 3 到 6 条，引用原话并写出它违反的是哪一条规则；一条都没有发现就写明「第二遍无发现」。第二遍必须给出结论，没有结论不能做第三遍。
...
先把「判断顺序」第一道的语境（含整篇结构体检、写下语境读数）和第二道的句子体检做完，再扫下面这张表。这张表只抓已知的词，抓不到没主语、没动词、虚词被压掉的句子，也判断不了同一个字在这个语境里该展开成哪个词。
...
- performative hype words: `扒`、`炸` / `有点炸` / `炸裂`、`砸`、`甩`、`秒`、`绝了`、`起飞`、`杀疯了`、`封神`、`撂`（撂下一句）、`一口气`、`狠事` / `最狠的`（`狠`单用可留）、`离谱`、`逆天`（accurate tech words like `锁`/`跑通` are fine；`写死`/`锁死` 这一族已永久禁用，见下一行）
...
- 「动词 + 死」一族（任何语境，黄佬永久禁用）: `写死` / `锁死` / `焊死` / `钉死` / `封死` / `定死` / `咬死`——改成 `固定成` / `固定住` / `全程不动` / `定下来不再改` / `一条条定好`，hardcode 可以直接写英文原词；单用的 `锁` / `加锁` 保留
- truncated words（用词要说完整）: `没变` / `没动` / `单价没动` / `价格没动` / `硬货` / `真货` / `干货` / `不拦` / `没改` / `照旧` / `不变`（不拦 → 不会拦截；改成 `没变化` / `没有变化` / `价格没有变化`，"硬货"直接说是数字还是配置）
- telegraphic clauses（电报腔，逐句读出声才抓得到）: `一段脚本进去` / `位置得我自己猜` / `X 出人声，Y 拆音效` 这类没主语或没动词的压缩短句——改成饭桌上讲得出口的完整句子，谁、做了什么、对什么做的
...
- truncated verbs（双音节动词砍成单字）: `删了` / `装了` / `改了` / `测了` / `查了` / `配好` / `传完` / `存好`，以及单字动词直接带宾语（`装依赖` / `删目录` / `改配置`）——按语境改成 `删除了` / `安装了` / `修改了` / `测试了` / `检查了` 或 `查询了` / `配置好` / `上传完` / `保存好`；`删掉` / `改散` / `调小` / `查清楚` 这类带补语的保留，`查表` / `删库` / `调参` / `压测` 这类术语保留，`跑` / `跑通` / `拉到本地` / `锁` 不是缩写、保留（`写死` / `锁死` 这一族已永久禁用，不在保留之列）
...
- squeezed-out function words（虚词被压掉）: 这一条查不了字符串，要逐句念出声——`位置得我自己猜` 念出来会多一个字（`还` / `只能`），`猜错半秒整段重新混一遍` 少了 `就得`。常被压掉的是 `还` / `只能` / `才` / `就` / `就得` / `都` / `也` / `又` / `非得` / `根本` / `去`；连动式缺 `去` 单列一类（`听着估摸` → `听着去估摸`，`盯着调整` → `盯着去调整`；`笑着说` / `躺着看` 这类交代方式的不用加）
- 只剩名词加趋向词、动词整个不见的残句: `一段脚本进去` / `三个参数进去，结果出来`（把动作和主语补回来）
- 整篇结构体检：开头承诺几条、后面兑现几节，对不对得上；主论收完之后有没有多余的收尾段；有没有连续 20 字以上的逐字重复
- 语境读数有没有写下来（口语场合 / 书面场合），第二道第 2 问和 truncated verbs 那条有没有按它判
